...
export default function AnimatedHero({ onStartPlanning, onExploreDemo, onResumeLastPlan, canResume }: AnimatedHeroProps) {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[88vh] overflow-hidden rounded-3xl border border-brand/30 bg-stone-950 p-6 text-white shadow-soft md:p-10">
      ...
      <div className="relative z-10 grid min-h-[78vh] items-center gap-8 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-semibold text-stone-900" onClick={onStartPlanning}>
              Start Planning
              <ArrowRight size={14} />
            </button>
            <button className="rounded-xl border border-white/25 bg-white/10 px-4 py-2 text-sm font-semibold text-white" onClick={onExploreDemo}>
              Explore Demo
            </button>
            {canResume && onResumeLastPlan && (
              <button className="rounded-xl border border-cyan-300/30 bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-100" onClick={onResumeLastPlan}>
                Resume Last Plan
              </button>
            )}
          </div>
