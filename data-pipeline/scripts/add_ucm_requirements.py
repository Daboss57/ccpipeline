"""
Add UC Merced major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UC Merced majors to reusable
lower-division requirement templates, then creates LPC -> UCM articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_ucm_requirements.py
"""
...
def run() -> None:
    ...
    ucm_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "ucm"}

    old_ucm_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "ucm"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "ucm"
    ]
    print(f"Removed {len(old_ucm_req_ids)} old UCM requirement entries")

    old_ucm_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "ucm"
    ...
    print(f"Removed {old_ucm_req_arts} old UCM REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCM courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    target_major_ids = sorted(ucm_major_id for ucm_major_id in ucm_majors.keys())
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCM)",
    ...
    print(f"Mapped {mapped} UCM majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCM articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "ucm"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCM requirement entries: {len(total_reqs)}")
    print(f"  UCM majors with requirements: {len(majors_with_reqs)} / {len(ucm_majors)}")
