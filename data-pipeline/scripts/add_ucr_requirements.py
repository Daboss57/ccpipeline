"""
Add UC Riverside major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UC Riverside majors to reusable
lower-division requirement templates, then creates LPC -> UCR articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_ucr_requirements.py
"""
...
def run() -> None:
    ...
    ucr_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "ucr"}

    old_ucr_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "ucr"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "ucr"
    ]
    print(f"Removed {len(old_ucr_req_ids)} old UCR requirement entries")

    old_ucr_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "ucr"
    ...
    print(f"Removed {old_ucr_req_arts} old UCR REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCR courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    target_major_ids = sorted(ucr_major_id for ucr_major_id in ucr_majors.keys())
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCR)",
    ...
    print(f"Mapped {mapped} UCR majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCR articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "ucr"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCR requirement entries: {len(total_reqs)}")
    print(f"  UCR majors with requirements: {len(majors_with_reqs)} / {len(ucr_majors)}")
