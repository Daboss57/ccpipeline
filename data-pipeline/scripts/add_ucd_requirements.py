"""
Add UC Davis major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UC Davis majors to reusable
lower-division requirement templates, then creates LPC -> UCD articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_ucd_requirements.py
"""
...
def run() -> None:
    ...
    ucd_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "ucd"}

    old_ucd_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "ucd"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "ucd"
    ]
    print(f"Removed {len(old_ucd_req_ids)} old UCD requirement entries")

    old_ucd_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "ucd"
    ...
    print(f"Removed {old_ucd_req_arts} old UCD REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCD courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    assist_major_ids = {
        m["major_id"]
        for m in ucd_majors.values()
    ...
    target_major_ids = sorted(assist_major_ids.union({mid for mid in seeded_ids if mid in ucd_majors}))
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCD)",
    ...
    print(f"Mapped {mapped} UCD majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCD articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "ucd"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCD requirement entries: {len(total_reqs)}")
    print(f"  UCD majors with requirements: {len(majors_with_reqs)} / {len(ucd_majors)}")
