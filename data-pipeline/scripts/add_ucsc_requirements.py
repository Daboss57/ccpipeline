"""
Add UC Santa Cruz major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UC Santa Cruz majors to reusable
lower-division requirement templates, then creates LPC -> UCSC articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_ucsc_requirements.py
"""
...
def run() -> None:
    ...
    ucsc_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "ucsc"}

    old_ucsc_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "ucsc"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "ucsc"
    ]
    print(f"Removed {len(old_ucsc_req_ids)} old UCSC requirement entries")

    old_ucsc_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "ucsc"
    ...
    print(f"Removed {old_ucsc_req_arts} old UCSC REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCSC courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    target_major_ids = sorted(ucsc_major_id for ucsc_major_id in ucsc_majors.keys())
    ...
    for major_id in target_major_ids:
        major = ucsc_majors.get(major_id)
        if not major:
            continue
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCSC)",
    ...
    print(f"Mapped {mapped} UCSC majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCSC articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "ucsc"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCSC requirement entries: {len(total_reqs)}")
    print(f"  UCSC majors with requirements: {len(majors_with_reqs)} / {len(ucsc_majors)}")
