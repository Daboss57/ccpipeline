"""
Add UC Santa Barbara major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UC Santa Barbara majors to reusable
lower-division requirement templates, then creates LPC -> UCSB articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_ucsb_requirements.py
"""
...
def run() -> None:
    ...
    ucsb_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "ucsb"}

    old_ucsb_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "ucsb"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "ucsb"
    ]
    print(f"Removed {len(old_ucsb_req_ids)} old UCSB requirement entries")

    old_ucsb_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "ucsb"
    ...
    print(f"Removed {old_ucsb_req_arts} old UCSB REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCSB courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    target_major_ids = sorted(ucsb_major_id for ucsb_major_id in ucsb_majors.keys())
    ...
    for major_id in target_major_ids:
        major = ucsb_majors.get(major_id)
        if not major:
            continue
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCSB)",
    ...
    print(f"Mapped {mapped} UCSB majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCSB articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "ucsb"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCSB requirement entries: {len(total_reqs)}")
    print(f"  UCSB majors with requirements: {len(majors_with_reqs)} / {len(ucsb_majors)}")
