"""
Add UC Irvine major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UC Irvine majors to reusable
lower-division requirement templates, then creates LPC -> UCI articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_uci_requirements.py
"""
...
def run() -> None:
    ...
    uci_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "uci"}

    old_uci_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "uci"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "uci"
    ]
    print(f"Removed {len(old_uci_req_ids)} old UCI requirement entries")

    old_uci_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "uci"
    ...
    print(f"Removed {old_uci_req_arts} old UCI REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCI courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    target_major_ids = sorted(uci_major_id for uci_major_id in uci_majors.keys())
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCI)",
    ...
    print(f"Mapped {mapped} UCI majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCI articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "uci"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCI requirement entries: {len(total_reqs)}")
    print(f"  UCI majors with requirements: {len(majors_with_reqs)} / {len(uci_majors)}")
