"""
Add UCLA major requirements to seed_data.json.

This script mirrors the UCSD pipeline and maps UCLA majors to reusable
lower-division requirement templates, then creates LPC -> UCLA articulation
rows for each requirement.

Run:
    python data-pipeline/scripts/add_ucla_requirements.py
"""
...
def run() -> None:
    ...
    ucla_majors = {m["major_id"]: m for m in data.get("majors", []) if m.get("school_id") == "ucla"}

    old_ucla_req_ids = {
        row["requirement_id"]
        for row in data.get("major_requirements", [])
        if row.get("university_id") == "ucla"
    }
    data["major_requirements"] = [
        row for row in data.get("major_requirements", []) if row.get("university_id") != "ucla"
    ]
    print(f"Removed {len(old_ucla_req_ids)} old UCLA requirement entries")

    old_ucla_req_arts = 0
    retained_arts = []
    for row in data.get("assist_articulations", []):
        if (
            row.get("university_id") == "ucla"
    ...
    print(f"Removed {old_ucla_req_arts} old UCLA REQ articulation entries")
    ...
    print(f"Added {new_courses} new UCLA courses, {new_offerings} offerings, {new_prereqs} prerequisites")

    target_major_ids = sorted(uccla_major_id for uccla_major_id in ucla_majors.keys())
    ...
    for major_id in target_major_ids:
        major = ucla_majors.get(major_id)
        if not major:
            continue
            ...
            data.setdefault("major_requirements", []).append(
                {
                    ...
                    "source_name": "ASSIST.org Articulation Agreement (LPC → UCLA)",
    ...
    print(f"Mapped {mapped} UCLA majors -> {new_reqs} requirement entries")
    print(f"Added {new_arts} LPC -> UCLA articulation entries")
    ...
    total_reqs = [r for r in data.get("major_requirements", []) if r.get("university_id") == "ucla"]
    majors_with_reqs = {r["major_id"] for r in total_reqs}
    print("Final totals:")
    print(f"  UCLA requirement entries: {len(total_reqs)}")
    print(f"  UCLA majors with requirements: {len(majors_with_reqs)} / {len(ucla_majors)}")
